...
class StickInput:
    ...
    def __add__(self, other: Self) -> Self:
        if self == other:
            ...
            y = self.y + other.y
            d = math.sqrt(x * x + y * y)
            return StickInput(x / d, y / d)
    
    def __sub__(self, other: Self) -> Self:
        if other.x == 0.0 and other.y == 0.0:
            return self
        else:
            return self + StickInput(-other.x, -other.y)
...
class GamepadInput:
    ...
    #     if self.seconds < 0.0:
    #         raise ValueError(f"Invalid seconds: {self.seconds}. must be non-negative.")

    def __add__(self, other: Self) -> Self:
        # if self.seconds != other.seconds: raise ValueError()
        return GamepadInput(
            self.seconds,
